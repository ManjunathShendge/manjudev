...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    // LOCAL CHANGE (upstream races here): loadAssets() is async, so under
    // StrictMode's double-invoke the first app is disposed — which calls
    // forceContextLoss() — before its init() runs. initPasses() then asks the
    // dead context for getContextAttributes(), gets null, and throws on
    // `.alpha`. Skip init if this instance was already torn down.
